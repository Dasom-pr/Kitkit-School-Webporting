...
import { QuickFactsEngine } from '../../game/quickfacts/QuickFactsEngine'
...
export default function QuickFactsPage() {
  ...
  const engineRef = useRef<QuickFactsEngine | null>(null)
  ...
  useEffect(() => {
    ...
    if (!canvas) return
    const engine = new QuickFactsEngine(canvas, level)
  ...
  if (level === 0) {
    return (
      <div style={{ background: '#1A237E', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <h1 style={{ color: '#FFD600', fontSize: 48, marginBottom: 32 }}>💎 Quick Facts</h1>
