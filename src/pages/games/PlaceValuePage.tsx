...
import { PlaceValueEngine } from '../../game/placevalue/PlaceValueEngine'
...
export default function PlaceValuePage() {
  ...
  const engineRef = useRef<PlaceValueEngine | null>(null)
  ...
  useEffect(() => {
    ...
    if (!canvas) return
    const engine = new PlaceValueEngine(canvas, level)
  ...
  if (level === 0) {
    return (
      <div style={{ background: '#1B5E20', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <h1 style={{ color: '#A5D6A7', fontSize: 48, marginBottom: 32 }}>💧 Place Value</h1>
