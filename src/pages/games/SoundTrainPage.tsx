...
import { SoundTrainEngine } from '../../game/soundtrain/SoundTrainEngine'
import ProgressBar from '../../components/ProgressBar'
import BackButton from '../../components/BackButton'
import { useShellParams } from '../../hooks/useShellParams'

const LEVEL_COLORS = ['#42A5F5','#66BB6A','#FFA726','#AB47BC','#EF5350','#26C6DA','#8D6E63','#78909C','#EC407A','#7E57C2','#29B6F6','#FF7043','#FFCA28','#81C784','#64B5F6','#E57373','#FF8A65','#A1887F','#90A4AE','#F48FB1','#80DEEA']

export default function SoundTrainPage() {
  const navigate = useNavigate()
  const { shellLevel, isFromShell, onGameComplete, shellBack } = useShellParams()
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const engineRef = useRef<SoundTrainEngine | null>(null)
  ...
  useEffect(() => {
    fetch('/data/games/soundtrain.json').then(r=>r.json()).then(data => {
      setAvailableLevels((data.levels||[]).map((l:{level:number})=>l.level).sort((a:number,b:number)=>a-b))
    }).catch(()=>setAvailableLevels([]))
  }, [])

  const startLevel = useCallback((lvl: number) => { setLevel(lvl); setShowComplete(false); setProgress({current:0,max:6}) }, [])

  useEffect(() => {
    if (level === 0) return
    const canvas = canvasRef.current; if (!canvas) return
    const engine = new SoundTrainEngine(canvas, level)
  ...
  if (level === 0) return (
    <div style={{width:'100vw',height:'100vh',background:'linear-gradient(135deg,#4facfe,#00f2fe)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:24}}>
      <BackButton color="#fff" />
      <h1 style={{color:'#fff',fontSize:40,fontWeight:'bold',textShadow:'2px 2px 6px rgba(0,0,0,0.3)',margin:0}}>🚂 Sound Train</h1>
      <p style={{color:'rgba(255,255,255,0.85)',fontSize:20,margin:0}}>Listen to the sound and pick the right letter!</p>
  ...
  return (
    <div style={{width:'100vw',height:'100vh',position:'relative',overflow:'hidden'}}>
      ...
      {showComplete && <CompleteOverlay onReplay={()=>startLevel(level)} onLevels={()=>setLevel(0)} onHome={()=>isFromShell?shellBack():navigate('/')} />}
    </div>
  )
}

function CompleteOverlay({onReplay,onLevels,onHome}:{onReplay:()=>void,onLevels:()=>void,onHome:()=>void}) {
  return (
    <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.55)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',zIndex:200,gap:28}}>
      ...
      <div style={{display:'flex',gap:16}}>
        <button onClick={onReplay} style={btnStyle('#4CAF50')}>Play Again</button>
        <button onClick={onLevels} style={btnStyle('#2196F3')}>Other Levels</button>
        <button onClick={onHome} style={btnStyle('#FF5722')}>Home</button>
      </div>
    </div>
  )
}
