...
import { ThirtyPuzzleEngine } from '../../game/thirtypuzzle/ThirtyPuzzleEngine'
...
export default function ThirtyPuzzlePage() {
  ...
  const engineRef = useRef<ThirtyPuzzleEngine | null>(null)
  ...
  useEffect(() => {
    ...
    if (!canvas) return
    const engine = new ThirtyPuzzleEngine(canvas, level)
  ...
  if (level === 0) {
    return (
      <div style={{ background: '#E65100', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <h1 style={{ color: '#FFF3E0', fontSize: 48, marginBottom: 32 }}>🔢 30 Puzzle</h1>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12, maxWidth: 600 }}>
          {Array.from({ length: TOTAL_LEVELS }, (_, i) => (
            <button key={i + 1} onClick={() => startLevel(i + 1)} style={{
              background: 'rgba(255,255,255,0.1)', border: '2px solid #FFF3E0',
              color: '#FFF3E0', borderRadius: 12, padding: '12px 0', fontSize: 22, cursor: 'pointer',
            }}>
              {i + 1}
            </button>
          ))}
    ...
    <div style={{ position: 'relative', width: '100vw', height: '100vh', background: '#000', overflow: 'hidden' }}>
      ...
      {showComplete && (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ color: '#FFF3E0', fontSize: 48, marginBottom: 24 }}>Level Complete!</div>
          <button onClick={isFromShell ? shellBack : () => { setShowComplete(false); setLevel(0) }} style={{
            background: '#FFF3E0', color: '#E65100', border: 'none', borderRadius: 16,
