...
import { NumberTracingExtEngine } from '../../game/numbertracingext/NumberTracingExtEngine'
import { useShellParams } from '../../hooks/useShellParams'

const TOTAL_LEVELS = 21

export default function NumberTracingExtPage() {
  const navigate = useNavigate()
  const { shellLevel, isFromShell, onGameComplete, shellBack } = useShellParams()
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const engineRef = useRef<NumberTracingExtEngine | null>(null)
  ...
  useEffect(() => {
    ...
    if (!canvas) return
    const engine = new NumberTracingExtEngine(canvas, level)
  ...
  if (level === 0) {
    return (
      <div style={{ background: '#1565C0', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <h1 style={{ color: '#E3F2FD', fontSize: 48, marginBottom: 32 }}>✏️ Number Tracing Extended</h1>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 12, maxWidth: 700 }}>
          {Array.from({ length: TOTAL_LEVELS }, (_, i) => (
            <button key={i + 1} onClick={() => startLevel(i + 1)} style={{
              background: 'rgba(255,255,255,0.1)', border: '2px solid #E3F2FD',
              color: '#E3F2FD', borderRadius: 12, padding: '12px 0', fontSize: 22, cursor: 'pointer',
            }}>
              {i + 1}
            </button>
          ))}
    ...
    <div style={{ position: 'relative', width: '100vw', height: '100vh', background: '#000', overflow: 'hidden' }}>
      ...
      {showComplete && (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ color: '#E3F2FD', fontSize: 48, marginBottom: 24 }}>Level Complete!</div>
          <button onClick={isFromShell ? shellBack : () => { setShowComplete(false); setLevel(0) }} style={{
            background: '#E3F2FD', color: '#1565C0', border: 'none', borderRadius: 16,
