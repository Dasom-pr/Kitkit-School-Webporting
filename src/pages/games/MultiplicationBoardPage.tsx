...
import { MultiplicationBoardEngine } from '../../game/multiplicationboard/MultiplicationBoardEngine'
...
export default function MultiplicationBoardPage() {
  ...
  const engineRef = useRef<MultiplicationBoardEngine | null>(null)
  ...
  useEffect(() => {
    ...
    if (!canvas) return
    const engine = new MultiplicationBoardEngine(canvas, level)
  ...
  if (level === 0) {
    return (
      <div style={{ background: '#1A237E', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <h1 style={{ color: '#FFD600', fontSize: 48, marginBottom: 32 }}>💡 Multiplication Board</h1>
