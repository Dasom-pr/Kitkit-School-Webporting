...
import { LabelingEngine } from '../../game/labeling/LabelingEngine'
...
export default function LabelingPage() {
  ...
  const engineRef = useRef<LabelingEngine | null>(null)
  ...
  useEffect(() => {
    fetch('/data/games/labeling.json').then(r=>r.json()).then(data => {
  ...
  useEffect(() => {
    ...
    const engine = new LabelingEngine(canvas, level)
  ...
  if (level === 0) return (
    <div style={{width:'100vw',height:'100vh',background:'linear-gradient(135deg,#667eea,#764ba2)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:24}}>
      ...
      <h1 style={{color:'#fff',fontSize:40,fontWeight:'bold',textShadow:'2px 2px 6px rgba(0,0,0,0.3)',margin:0}}>Labeling</h1>
      <p style={{color:'rgba(255,255,255,0.85)',fontSize:20,margin:0}}>Tap a word, then tap its spot on the image!</p>
