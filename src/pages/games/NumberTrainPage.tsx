...
import { NumberTrainEngine } from '../../game/numbertrain/NumberTrainEngine'
...
export default function NumberTrainPage() {
  ...
  const engineRef = useRef<NumberTrainEngine | null>(null)
  ...
  useEffect(() => {
    ...
    if (!canvas) return
    const engine = new NumberTrainEngine(canvas, level)
  ...
  if (level === 0) {
    return (
      <div style={{ background: '#1a237e', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <h1 style={{ color: '#FFD600', fontSize: 48, marginBottom: 32 }}>🚂 Number Train</h1>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, maxWidth: 500 }}>
          {Array.from({ length: TOTAL_LEVELS }, (_, i) => (
            <button key={i + 1} onClick={() => startLevel(i + 1)} style={{
              background: 'rgba(255,255,255,0.1)', border: '2px solid #FFD600',
              color: '#FFD600', borderRadius: 12, padding: '12px 0', fontSize: 22, cursor: 'pointer',
            }}>
              {i + 1}
            </button>
          ))}
    ...
    <div style={{ position: 'relative', width: '100vw', height: '100vh', background: '#000', overflow: 'hidden' }}>
      ...
      {showComplete && (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ color: '#FFD600', fontSize: 48, marginBottom: 24 }}>Level Complete!</div>
          <button onClick={isFromShell ? shellBack : () => { setShowComplete(false); setLevel(0) }} style={{
            background: '#FFD600', color: '#1a237e', border: 'none', borderRadius: 16,
