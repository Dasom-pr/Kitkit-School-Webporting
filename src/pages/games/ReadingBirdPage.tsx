...
import { ReadingBirdEngine } from '../../game/readingbird/ReadingBirdEngine'
...
export default function ReadingBirdPage() {
  ...
  const engineRef = useRef<ReadingBirdEngine | null>(null)
  ...
  useEffect(() => {
    fetch('/data/games/readingbird.json').then(r=>r.json()).then(data => {
  ...
  useEffect(() => {
    ...
    const engine = new ReadingBirdEngine(canvas, level)
  ...
  if (level === 0) return (
    <div style={{width:'100vw',height:'100vh',background:'linear-gradient(135deg,#667eea,#764ba2)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:24}}>
      ...
      <h1 style={{color:'#fff',fontSize:40,fontWeight:'bold',textShadow:'2px 2px 6px rgba(0,0,0,0.3)',margin:0}}>Reading Bird</h1>
      <p style={{color:'rgba(255,255,255,0.85)',fontSize:20,margin:0}}>Listen to the word and identify it!</p>
