...
export class NumberTraceEngine extends BaseEngine {
  ...
  async loadInsectFrames(assetType: string) {
    ...
    const loadFrames = async (suffix: string, total: number, maxLoad: number) => {
      ...
      return results.filter(img => img.complete && img.naturalWidth > 0)
    }
  ...
  draw() {
    ...
    // No text hints — matches C++ (no UI overlays)
  ...
  drawInsects(gs: number) {
    ...
    for (const ins of this.insects) {
      ...
      if (!frame || !frame.complete || frame.naturalWidth === 0) continue

      ctx.save()
      ctx.globalAlpha = ins.fadeAlpha

      // Compute draw position based on animation state
      let drawX = ins.x
      let drawY = ins.y
      let drawRotation = ins.rotation

      if (ins.moveInProgress < 1 && ins.moveInProgress > 0) {
        // Move-in: quadratic Bezier curve (approximates C++ CircleAndStraight)
        const t = ins.moveInProgress
        // Ease-out for deceleration at rest position
  ...
  drawCursor(gs: number) {
    ...
    if (!this.cursorImage?.complete || this.cursorImage.naturalWidth === 0) return
    ...
    const imgSize = this.cursorImage.naturalWidth  // 118px
