...
export class PatternTrainEngine extends BaseEngine {
  ...
  buildTrain(pattern: string[], freightCarCount: number, blank: Blank, shape: Shape) {
    ...
    const locoW = locoImg?.complete ? locoImg.naturalWidth : 455
    const locoH = locoImg?.complete ? locoImg.naturalHeight : 446
    ...
    for (let i = 0; i < freightCarCount; i++) {
      ...
      const carW = carImg?.complete ? carImg.naturalWidth : (carType === 2 ? 467 : 706)
      const carH = carImg?.complete ? carImg.naturalHeight : 97
