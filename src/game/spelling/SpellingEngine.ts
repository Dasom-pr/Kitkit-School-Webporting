...
export class SpellingEngine extends BaseEngine {
  ...
  draw() {
    ...
    if (this.wordImage?.complete) {
      const wiLeft = this.gbLeft + 110
      const wiTop = this.gbTop + 80  // GAMEBOARD_H - (GAMEBOARD_H - 80) = 80 from gbTop
    ...
    for (const slot of this.slots) {
      ...
      if (this.frameHoleImage?.complete) {
    ...
    for (let i = 0; i < this.slots.length; i++) {
      ...
      if (this.crystalYellow?.complete) {
