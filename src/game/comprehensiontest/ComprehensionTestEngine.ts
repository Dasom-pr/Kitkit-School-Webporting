...
export class ComprehensionTestEngine extends BaseEngine {
  ...
  draw() {
    ...
    if (this.papersBottomImage.complete) {
    ...
    if (this.papersTopImage.complete) {
  ...
  drawChoices(gs: number) {
    ...
    for (let i = 0; i < this.choiceButtons.length; i++) {
      ...
      if (btn.state === 'correct' && this.choiceCorrectImage.complete) {
        useImage = this.choiceCorrectImage
      } else if (btn.state === 'wrong' && this.choiceWrongImage.complete) {
        useImage = this.choiceWrongImage
      } else if (this.choiceNormalImage.complete) {
