...
export default class HundredPuzzleEngine extends BaseEngine {
  ...
  setupProblem() {
    ...
    // C++ places pieces to the right of the board: x in [boardRight, gameSize.width-pieceW]
    // In web, we place choice buttons below the board instead (simplified interaction)
    const choiceBtnW = 138
    const choiceBtnH = 138
    const choiceGap = 16
    const totalChoiceW = choiceValues.length * (choiceBtnW + choiceGap) - choiceGap
    const choiceStartX = (GAME_WIDTH - Math.min(totalChoiceW, GAME_WIDTH - 100)) / 2
    // Position choices below board with some spacing
    const choiceY = GAME_HEIGHT - 200

    for (let i = 0; i < choiceValues.length; i++) {
      this.choices.push({
        value: choiceValues[i],
        x: choiceStartX + i * (choiceBtnW + choiceGap),
        y: choiceY,
        width: choiceBtnW,
        height: choiceBtnH,
      })
  ...
  draw() {
    ...
    if (this.boardImage.complete && this.cells.length > 0) {
      const boardCols = 10
  ...
  drawChoices(gs: number) {
    ...
    ctx.textAlign = 'center'
    ctx.textBaseline = 'middle'
    ctx.fillText('Choose a number:', GAME_WIDTH / 2 * gs, (GAME_HEIGHT - 310) * gs)
