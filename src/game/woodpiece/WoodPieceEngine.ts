...
import { BaseEngine, GAME_WIDTH as GW, GAME_HEIGHT as GH, loadImage, loadAudio, playSound } from '../common/BaseEngine'
...
export class WoodPieceEngine extends BaseEngine {
  ...
  updateBoardLayout() {
    if (!this.boardImg?.complete || this.boardNatW > 0) return
    ...
    if (isOrdered) {
      ...
      const scale = Math.min((GW - 40) / this.boardNatW, (GH - 40) / this.boardNatH)
      this.boardW = this.boardNatW * scale
      this.boardH = this.boardNatH * scale
  ...
  computePieceTargets() {
    ...
    for (const piece of this.pieces) {
      ...
      const pw = piece.faceImg.complete ? piece.faceImg.width * scaleX : piece.width * scaleX
      const ph = piece.faceImg.complete ? piece.faceImg.height * scaleY : piece.height * scaleY
      piece.width = pw
      piece.height = ph
  ...
  update(_time: number, dt: number) {
    ...
    if (this.boardNatW > 0) {
      ...
      for (const p of this.pieces) {
        if (p.faceImg.complete && p.width !== p.faceImg.width * (this.boardW / this.boardNatW)) {
          needsRecompute = true
          break
        }
      }
  ...
  draw() {
    ...
    // Board image
    if (this.boardImg?.complete && this.boardNatW > 0) {
    ...
    for (const piece of this.pieces) {
      if (piece.slotImg.complete && this.boardNatW > 0) {
  ...
  drawPlacedPiece(piece: Piece, gs: number) {
    const { ctx } = this
    if (!piece.faceImg.complete) return
  ...
  drawPiece3D(piece: Piece, gs: number) {
    ...
    if (piece.dragging) {
      ...
      if (piece.shadowImg.complete) {
        ctx.drawImage(piece.shadowImg, bx + 6 * gs, by + 6 * gs, pw, ph)
      }
      if (piece.depthImg.complete) {
