...
export class SentenceBridgeEngine extends BaseEngine {
  ...
  async loadData() {
    ...
    const carW = carImg?.complete ? carImg.naturalWidth : 250
    ...
    for (const p of pieces) {
      if (p.isSlot) {
        ...
        const bgImg = p.multilineY === -1 ? this.imgSlotBg2Top
          ...
          : this.imgSlotBg1
        p.width = bgImg.complete ? bgImg.naturalWidth : 180
      } else {
  ...
  onPointerMove(x: number, y: number) {
    ...
    for (const piece of prob.pieces) {
      ...
      const sw = slotImg.complete ? slotImg.naturalWidth : 100
      const sh = slotImg.complete ? slotImg.naturalHeight : 100
  ...
  onBridgeComplete() {
    ...
    const carW2 = carImg2?.complete ? carImg2.naturalWidth : 250
  ...
  spawnHeart() {
    ...
    const carTopY = BRIDGE_CENTER_Y - (this.imgBridgeBody.complete ? this.imgBridgeBody.naturalHeight / 2 : 246) + 20 - ch
  ...
  drawRails() {
    const ctx = this.ctx
    const bridgeBodyH = this.imgBridgeBody.complete ? this.imgBridgeBody.naturalHeight : 492

    if (this.imgRail.complete) {
  ...
  drawBridge(idx: number, baseX: number) {
    const ctx = this.ctx
    const prob = this.problems[idx]
    const bodyH = this.imgBridgeBody.complete ? this.imgBridgeBody.naturalHeight : 492

    // Bridge body (stretched to bridgeWidth)
    if (this.imgBridgeBody.complete) {
    ...
    // Left pillar
    if (this.imgPillarLeft.complete) {
    ...
    if (idx === this.problems.length - 1 && this.imgPillarRight.complete) {
  ...
  drawPiece(piece: SentencePiece, bridgeX: number) {
    ...
    if (piece.isSlot) {
      ...
      const bgImg = piece.multilineY === -1 ? this.imgSlotBg2Top
        ...
        : this.imgSlotBg1
      if (bgImg.complete && piece.bgFadeAlpha > 0) {
      ...
      // Capital mark
      if (piece.isUppercase && this.imgCapitalMark.complete) {
