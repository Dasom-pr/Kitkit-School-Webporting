...
export class TutorialTraceEngine extends BaseEngine {
  ...
  private drawCursor(gs: number) {
    ...
    if (!this.cursorImg?.complete || this.cursorImg.naturalWidth === 0) return
    ...
    const imgSize = this.cursorImg.naturalWidth  // 130px

    // C++: cursor is child of GlyphNode at (0,0) — position = trace point directly
