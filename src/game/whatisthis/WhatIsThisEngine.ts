...
export class WhatIsThisEngine extends BaseEngine {
  ...
  private drawTopCard(card: CardState, isFail: boolean): void {
    ...
    if (card.cardType === 'image') {
      ...
      if (this.topImgMask.complete && card.data.pic) {
  ...
  private drawSpeakerIcon(card: CardState): void {
    ...
    const ctx = this.ctx
    const bgW = this.topTextBg.complete ? this.topTextBg.naturalWidth : 2208
    const bgH = this.topTextBg.complete ? this.topTextBg.naturalHeight : 340
  ...
  private drawDownCard(card: CardState): void {
    ...
    if (card.showBack) {
      ...
      if (card.cardType === 'image') {
        ...
        // Wrong BG overlay
        if (card.wrongOpacity > 0 && this.downImgFrontWrong.complete) {
          ctx.save()
        ...
        if (this.downImgMask.complete && card.data.pic) {
        ...
        // Wrong BG overlay
        if (card.wrongOpacity > 0 && this.downTextFrontWrong.complete) {
          ctx.save()
          ctx.globalAlpha = card.wrongOpacity * card.opacity
        ...
        // Text label
        const bgW = this.downTextFront.complete ? this.downTextFront.naturalWidth : 2208
