...
export class EggQuizEngine extends BaseEngine {
  ...
  drawMathQuestion(p: MathProblem, gs: number) {
    ...
    switch (qc.mode) {
      case 'counting': {
        ...
        if (count <= 9 && this.imgAcorn.complete && qc.items && qc.items.length > 0) {
          ...
          for (let s = 0; s < sections; s++) {
            ...
            if (tallyIdx >= 0 && tallyIdx < this.imgTally.length && this.imgTally[tallyIdx].complete) {
      ...
      case 'missing': {
        ...
        for (let i = 0; i < numbers.length; i++) {
          ...
          if (i === blankIdx) {
            ...
            if (this.imgEmptyLarge.complete) {
  ...
  drawPopup(gs: number) {
    ...
    if (this.popupState === 'pass') {
      ...
      // Ribbon back
      if (this.imgRibbonBack.complete) {
        this.drawImg(this.imgRibbonBack, GW / 2, GH / 2 - 200, 800, 0)
      }

      // Egg or Bird image
      const charImg = this.imgBird?.complete ? this.imgBird : this.imgEgg
      if (charImg?.complete) {
        this.drawImg(charImg, GW / 2, GH / 2 - 50, 0, 500)
      }

      // Ribbon front
      if (this.imgRibbonFront.complete) {
