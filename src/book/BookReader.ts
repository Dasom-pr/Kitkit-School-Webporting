...
export function parseBookInfoCSV(csv: string, bookId: string): Book {
  ...
  for (const line of lines) {
    ...
    if (type === 'title') {
      ...
      currentPage = {
        pageNumber: 0,
        imageFile: titleImageFile ? titleImageFile + '.png' : undefined,
        paragraphs: [],
      }
      ...
      currentPage = {
        pageNumber: pageNum,
        imageFile: imageBase ? imageBase + '.png' : undefined,
  ...
  return {
    title,
    titleAudioFile,
    titleImageFile: titleImageFile ? titleImageFile + '.png' : undefined,
    layout,
    hasAudio,
